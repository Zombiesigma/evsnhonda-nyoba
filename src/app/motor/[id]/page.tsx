...
import { ChevronLeft, Plus, Layers, FileText, CheckCircle2 } from 'lucide-react';
...
export default function MotorcycleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!bike) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center space-y-4 px-6 text-center">
      <h2 className="text-2xl font-bold tracking-tight">{t('detail_not_found')}</h2>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-10 md:gap-16 items-start">
          ...
          <div className="space-y-6 lg:sticky lg:top-24">
            <div className="relative aspect-[16/11] bg-zinc-50 rounded-[32px] overflow-hidden border border-zinc-100 shadow-sm group">
              <Image 
          ...
          <div className="space-y-10">
            <div className="space-y-4">
              <h1 className="text-4xl md:text-6xl font-extrabold tracking-tighter leading-none text-black">{bike.name}</h1>
            ...
            {/* Price Card */}
            <div className="bg-black text-white rounded-[24px] p-6 md:p-8 space-y-6 shadow-2xl shadow-black/20">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 mb-1 block">{t('detail_otr')}</span>
                ...
                {bike.variants && bike.variants.length > 1 && (
                  <div className="text-right">
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 mb-1 block">{t('detail_variant')}</span>
            ...
            <div className="pt-6">
               <Button className="w-full h-16 bg-black text-white rounded-2xl font-bold text-lg shadow-xl shadow-black/10 hover:scale-[1.02] transition-all" asChild>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(language === 'id' ? (bike.features_id || []) : (bike.features_en || [])).map((f: string, i: number) => (
                  <div key={i} className="flex items-center justify-between p-5 bg-white border border-zinc-100 rounded-2xl hover:border-black transition-all">
...
function DetailPageSkeleton() {
  return (
    <div className="min-h-screen pt-28 px-8 space-y-12 max-w-7xl mx-auto">
      <div className="grid lg:grid-cols-2 gap-16">
        <Skeleton className="aspect-[16/11] rounded-[32px]" />
        <div className="space-y-8">
